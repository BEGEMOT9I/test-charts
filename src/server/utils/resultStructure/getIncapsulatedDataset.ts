...
function createNestedSeria(series: Series, levels: Levels, level: number, labelsArray: Array<string>): NestedArray<number> {
  if (level) {
    return levels[levels.length - level - 1].labels.map((label, index) => createNestedSeria(series, levels, level - 1, labelsArray.concat(label))) as NestedArray<number>
  }

  const valueBasedOnParentArray = labelsArray.reduce((result, label, levelReverseIndex) => {
    const index = Number((label.match(/(index|seria)-(\d+)$/) as Array<string>)[2]) * 10 ** (labelsArray.length - levelReverseIndex)
